    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">Vendor Order List</li>
          </li>
        </ol>
      </div>
    </div>
...
if (!$this->session->userdata('user_id')) {
    redirect('signIn'); // Redirect to sign-in page if not logged in
}
...
if ($getVendorProduct !== null && $getVendorProduct->num_rows() > 0) {
    foreach ($getVendorProduct->result() as $r) {
        // Check if the resgister_id matches the logged-in user's ID and curr_date matches today
        $currDate = (new DateTime($r->curr_date))->format('Y-m-d');
        if ($r->resgister_id == $userId && $currDate == $today) {
