  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Vendor Approval List</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Vendor Approval List</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
             ...
             <!--  <div class="card-header">
              ...
              <div class="card-body">
                <table id="example2" class="table table-bordered table-striped">
                      ...
                      <?php
                      // Get today's date
                      $today = (new DateTime())->format('Y-m-d'); // 'Y-m-d' format for comparison
                      ?>

                      <?php if (!empty($vendor_data)): ?>
                          <?php $i = 1; ?>
                          <?php foreach ($vendor_data as $r): ?>
                              <?php
                              // Check if the account type is 'User'
                              if ($r->account_type == 'Vendor') {
                                  // Convert curr_date to Y-m-d format for comparison
                                  $currDate = (new DateTime($r->curr_date))->format('Y-m-d');
                                  
                                  // Display the row if the status is 'confirmed' and the date matches today
                                  // Or display if the status is not 'confirmed'
                                  if (($r->status == 'confirmed' && $currDate == $today) || $r->status != 'confirmed') {
                              ?>
                              <tr>
                                  <td><?php echo $i; ?></td>
                                  ...
                                  <td>
                                      ...
                                      <?php else: ?>
                                          <a class="btn btn-sm btn-warning confirm-btn" data-id="<?php echo $r->id; ?>">Confirm</a>
                                      <?php endif; ?>&nbsp;&nbsp;&nbsp;&nbsp;
                                      <a class="btn btn-sm btn-danger" onclick="setDeleteFunction('<?php echo $r->id; ?>')">Remove</a> 
                                  </td>
                              </tr>
                              <?php $i++; ?>
                              <?php } // End of the display check ?>
                              <?php } // End of user check ?>
                          <?php endforeach; ?>
                      <?php else: ?>
                          <tr>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form id="deleteForm" method="post" action="<?php echo base_url('welcome/removeVendor')?>">
...
$(document).ready(function() {
    $('.confirm-btn').on('click', function() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url('welcome/sendConfirmationEmail_vendor'); ?>/' + userId,
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success') {
                   ...
                   // location.reload();
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo base_url('welcome/updateStatus_vendor'); ?>/' + userId,
                        dataType: 'json',
